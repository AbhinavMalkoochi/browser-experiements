...
const PLAN_SCHEMA = {
  ...
  properties: {
    plan: { type: 'array', items: { type: 'string' }, minItems: 1, maxItems: 15 },
...
Use the accessibility snapshot's numbered refs. Never click a Submit button — emit {kind:"done", status:"ready_to_submit"}
when all required fields are filled and Submit is visible+enabled. If captcha/blocked → {kind:"abort", status:"blocked"}.
For file uploads use value:"resume".`;
...
export const approachD: Approach = {
  ...
  async run(ctx: ApproachCtx) {
    ...
    while (steps < ctx.maxSteps) {
      ...
      // Change-observer verifier: if two steps in a row produce no structural or value change, reset strategy.
      const diff = diffSnapshots(prevSnap, snap);
      if (steps > 1 && diff.changeRatio < 0.01) noChangeCount += 1;
      ...
      try {
        out = await chat<DslOutput>({
          ...
          messages: [
            ...
            { role: 'user', content: [planText, '', 'PROFILE:', profileYaml, '', 'CURRENT SNAPSHOT:', formatAx(snap, 140)].join('\n') },
          ],
      ...
      const res = await executeActions(ctx, snap, out.json.actions, steps);
